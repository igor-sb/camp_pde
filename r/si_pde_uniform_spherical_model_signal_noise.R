#
#   Fixed PDE concentration model with point cAMP source (spherical). 
#   Show how does PDE change gradient, mean concentration, signal, noise.
#
...
# Generate data frame with this function
...
snr.dt[, r0_labels := paste0('paste(r[0] == ', round(r0/1e-6), ', " ", mu, m)')]
snr.dt[, r0_labels := factor(r0_labels, levels=unique(r0_labels))]

# Calculate neccessary quantities
snr.dt[, c_front := c_sph(C0, r0 - 0.5*rc, p0)]
snr.dt[, c_back := c_sph(C0, r0 + 0.5*rc, p0)]
snr.dt[, c_mean := (c_front + c_back)/2]
snr.dt[, dcdx_mean := (c_front - c_back)/rc]
...
# Use analytic formulas to calculate c0_rel and dcdx_rel. Compare with numerical results.
snr.dt[, c0_rel_ana := exp(-r0/L(p0))]
snr.dt[, dcdx_rel_ana := exp(-r0/L(p0))*(r0/L(p0) + 1)]
...
x_range = -14:-9
y_range = -3:2
c_all.plot = ggplot(snr.dt[r0==0.000225 & C0==c0_range[17]], aes(x=p0, group=factor(C0), color=factor(C0))) +
    geom_line(mapping = aes(y=c0_rel_ana, linetype='dashed'), size = 1.5) +
    geom_line(mapping = aes(y=dcdx_rel_ana, linetype='dotted'), size=1.5) +
...
rec_dif.plot = ggplot(snr.dt[c0_over_kd %in% c0_plot], 
        aes(x=p0, y=delta_r, group=factor(c0_over_kd), color=factor(c0_over_kd))) +
    geom_line(size=1.5) +
...
#
rel_sig.plot = ggplot(snr.dt[c0_over_kd %in% c0_plot], 
                      aes(x=p0, y=delta_r_over_delta_r0, group=factor(c0_over_kd), color=factor(c0_over_kd))) +
    geom_line(size=1.5) +
    scale_x_continuous(name = expression(paste(P[0], ' [mol ', m^-2, s^{-1}, ']', sep='')), trans='log10',
                       breaks = 10^x_range, expand = c(0, 0),
                       labels = sapply(x_range, function (i) as.expression(bquote(10^ .(i))))) +
...
sigma.plot = ggplot(snr.dt[c0_over_kd %in% c0_plot], aes(x=p0, y=sigma_delta_r, group=factor(c0_over_kd), color=factor(c0_over_kd))) +
    geom_line(size=1.5) +
    scale_x_continuous(name = expression(paste(P[0], ' [mol ', m^-2, s^{-1}, ']', sep='')), trans='log10',
                       breaks = 10^x_range, expand = c(0, 0),
                       labels = sapply(x_range, function (i) as.expression(bquote(10^ .(i))))
...
x_range = -14:-9
y_range = -2:5
snr.plot = ggplot(snr.dt, aes(y = c0_over_kd, x = p0, z = snr, fill = snr)) +
...
snr_hor.plot = ggplot(snr.dt[c0_over_kd %in% c0_plot], aes(x=p0, y=snr, group=factor(c0_over_kd), color=factor(c0_over_kd))) +
    geom_line(size=1.5) +
    scale_x_continuous(
...
snr_ratio.plot = ggplot(snr.dt[snr_rel > 1e-4], aes(y=c0_over_kd, x=p0, z=snr_rel, fill=snr_rel)) +
    geom_raster(show.legend = T) +
...
#
pdf('../figures/si_pde_uniform_spherical_snr_signal_noise.pdf', useDingbats=F, width=14, height=11.7)
